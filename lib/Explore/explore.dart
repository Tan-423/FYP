...
class _MapViewScreenState extends State<MapViewScreen> {
  final String _googleMapsApiKey =
      "AIzaSyDBJEa2AqdPURKVJKFTDxAV7VJVQUtFkN4"; // ⚠️ Replace with your API key
  ...
  Future<void> _fetchLivePlacesFromGoogle(
    String filter, {
    String searchQuery = "",
  }) async {
    ...
    if (searchQuery.isNotEmpty) {
      url =
          'https://maps.googleapis.com/maps/api/place/textsearch/json?query=${Uri.encodeComponent(searchQuery)}&key=$_googleMapsApiKey';
    } else {
      ...
      url =
          'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${_currentLocation.latitude},${_currentLocation.longitude}&radius=$radius&type=$type&key=$_googleMapsApiKey';
    }
        ...
        if (data['status'] == 'OK') {
          ...
          for (var place in results) {
            ...
            if (place['photos'] != null && place['photos'].isNotEmpty) {
              ...
              photoUrl =
                  "https://maps.googleapis.com/maps/api/place/photo?maxwidth=600&photoreference=$photoRef&key=$_googleMapsApiKey";
            }
    ...
    setState(() {
      ...
      _allViewpoints = [
        Viewpoint(
          ...
          location: const LatLng(3.1556, 101.7144),
          tags: ["#nature"],
        ),
      ];
    });
  ...
  Future<void> _handleMapTap(LatLng latLng) async {
    ...
    if (_googleMapsApiKey != "YOUR_API_KEY_HERE" &&
        _googleMapsApiKey.isNotEmpty) {
      try {
        final url =
            'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${latLng.latitude},${latLng.longitude}&radius=30&key=$_googleMapsApiKey';
        final response = await http.get(Uri.parse(url));
        final data = json.decode(response.body);

        if (data['status'] == 'OK' && data['results'].isNotEmpty) {
          resolvedName = data['results'][0]['name'];
          if (data['results'][0]['types'] != null &&
              data['results'][0]['types'].isNotEmpty) {
            resolvedCategory =
    ...
    try {
      ...
      if (data['status'] == 'OK') {
        ...
        for (var step in leg['steps']) {
          String rawInstruction = step['html_instructions'].replaceAll(
            RegExp(r'<[^>]*>'),
            '',
          );
          ...
          parsedSteps.add(
            RouteStep(instruction: rawInstruction, points: points),
          );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Routing Failed: $googleError"),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Network error while calculating route."),
          backgroundColor: Colors.red,
        ),
      );
    }
  ...
  void _startLiveTracking() {
    ...
    _positionStream = Geolocator.getPositionStream(
      ...
      double bearing = Geolocator.bearingBetween(
        _carLocation!.latitude,
        _carLocation!.longitude,
        newPos.latitude,
        newPos.longitude,
      );
      ...
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        ...
        if (!snapshot.exists) {
          ...
          double currentTotalScore =
              (snapshot.data()?['totalScore'] ?? 0).toDouble();
            ...
            child:
                _isLoadingLocation
                    ...
                    : GoogleMap(
                      ...
                      onTap:
                          _handleMapTap, // ⭐ CONNECTED: Tapping anywhere builds a custom route!
                          ...
                          onTap: () {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                                  _isOfflineMode
                                      ? "Region downloaded for offline use."
                                      : "Reconnected to live map.",
                                ),
                              ),
                            );
                          },
                  ...
                  child: Column(
                    children: [
                      ...
                      if (_isLoadingPlaces)
                        ...
                        Expanded(
                          child: _buildExploreList(scrollController, isDark),
                        ),
                    ],
                  ),
                );
            ...
            children: [
              ...
              IconButton(
                ...
                onPressed:
                    () => setState(
                      () => _selectedViewpoint = null,
                    ), // 👈 Dismisses the custom drop pin easily!
              ),
            ],
          ),
          const SizedBox(height: 4),
          ...
          StreamBuilder<DocumentSnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasData && snapshot.data!.exists) {
                displayRating =
                    (snapshot.data!['rating'] ?? vp.rating).toDouble();
  ...
  Widget _buildFilterChip(String label, IconData icon) {
    bool isSelected =
        _selectedFilter == label && _searchController.text.isEmpty;
    ...
    return GestureDetector(
      ...
      child: Column(
        ...
        children: [
                ...
                Container(
                  decoration: BoxDecoration(
                    ...
                    image: DecorationImage(
                      image: NetworkImage(viewpoint.imageUrl),
                      fit: BoxFit.cover,
                    ),
                  ),
                ),
                  ...
                  child: Container(
                    ...
                    child: StreamBuilder<DocumentSnapshot>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        if (snapshot.hasData && snapshot.data!.exists) {
                          displayRating =
                              (snapshot.data!['rating'] ?? viewpoint.rating)
                                  .toDouble();
                        }
                        return Row(
                          children: [
          ...
          Text(
            "${viewpoint.category} • ${viewpoint.distance} km",
            style: TextStyle(
              fontSize: 12,
